...
class VideoQAAgent:
            ...
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        "Each reference must contain numeric start copied from a "
                        "transcript cue, timestamp (string), and text (non-empty "
                        "supporting subtitle or concise description)."
                    ),
                },
                {
                    "role": "user",
    ...
    def _format_cue(cue: Mapping[str, Any]) -> str:
        ...
        return f"[start={start:g}; timestamp={timestamp}] {text}"
